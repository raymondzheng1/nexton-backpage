"use client";
/**
 * "Where everyone finishes" — projected full-time minutes per player, drawn as a back-page bar
 * table: name, an ink bar on a 10% track, the red fair-share tick, minutes.
 *
...
import { FairBar, SectionHead, Status, cx, mins, styles } from "@/ui";
...
interface ProjectedMinutesProps {
  ...
  fairSeconds?: number;
  title?: string;
  /** Small uppercase caption on the section rule, e.g. "if the plan runs". */
  caption?: string;
...
export function ProjectedMinutes({
  ...
  return (
    <section>
      <SectionHead style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 10 }}>
        <span>{title}</span>
        {caption !== undefined && (
          <span
            style={{
      ...
      {rows.length === 0 ? (
        ...
        rows.map((row, i) => (
          <div key={row.id} className={cx(styles.row, i === rows.length - 1 && styles.rowLast)}>
            ...
            <FairBar pct={(row.seconds / max) * 100} fairPct={(fair / max) * 100} />
            <span style={{ width: 34, flex: "none", textAlign: "right", fontSize: 15, fontWeight: 800 }}>
              {mins(row.seconds)}
            </span>
            {/* A late or retired player is marked, never judged — a status glyph there would read as
                unfairness when their minutes simply aren't the plan's to give. */}
            <span style={{ width: 16, flex: "none", textAlign: "center" }} title={row.fixedReason || undefined}>
              {row.fixed ? <span style={{ color: "var(--ink-muted)", fontWeight: 800 }}>–</span> : <Status kind={row.status} wordless />}
            </span>
          </div>
        ))
      )}
      ...
      <p className={styles.note}>
        <span style={{ color: "var(--red)" }}>|</span> = fair share
