"use client";
/**
 * Corrections & clarifications — edit the club details, the roster, and the season fair-play rule.
 *
 * Same form as first run, seeded from the stored team, plus the two things only an existing team
 * has: carry-forward, and the delete. The team's stored colour is carried through untouched — the
 * Back Page has nothing to paint with it, but throwing away a field the record still holds would be
 * a lossy edit.
 */
...
import { Button, ButtonLink, Dateline, Masthead, SectionHead, cx, styles } from "@/ui";
...
export default function EditTeamPage() {
  ...
  if (!ready) {
    return (
      <main className={styles.page}>
        <Masthead slug="Corrections" back="/teams" />
  ...
  return (
    <main className={styles.page}>
      <Masthead slug="Corrections" back={`/teams/${team.id}`} />
      <Dateline left={team.name} right={`Squad of ${roster.length}`} />
      ...
      <div className={styles.gutter}>
        ...
        <SectionHead>Club details</SectionHead>
        <TeamForm value={basics} onChange={(patch) => setBasics((b) => ({ ...b, ...patch }))} />

        <SectionHead>Season fair play</SectionHead>
        ...
        <div className={css.foot}>
          <Button kind="ink" onClick={() => void save()} disabled={saving}>
